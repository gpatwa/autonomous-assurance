...
- **Week 3 — Day 2-5 NEXT.** polling-worker (Microsoft Graph audit polling, delta tokens). First real-tenant E2E (`patwainc.onmicrosoft.com` audit events → deployed pipeline → incident). Then week 4: API + /console + External ID + Slack notification.
