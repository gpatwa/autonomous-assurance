...
### Per-class `StateSnapshot` defaults (from WI-05 evidence)

`StateSnapshot.confidence` and `StateSnapshot.captureSource` are
populated per change class by the ingestion layer. The defaults below are
evidence-driven from WI-05 (2026-04-17 / 2026-04-18 test-tenant run,
`docs/SPIKE_REPORT_AUDIT_LOG_COMPLETENESS.md`, and the derived fixtures
in `platform/fixtures/canonical/`). These are **defaults**, not schema
constraints — the ingestion layer may downgrade to `best-effort` or
`unavailable` when evidence is missing for a specific event.

| Change class | `beforeState.confidence` | `afterState.confidence` | `beforeState.captureSource` | `afterState.captureSource` |
|---|---|---|---|---|
| Group membership | `reconstructed` | `authoritative` | `snapshot-diff` | `entra-audit` |
| Conditional Access policy | `authoritative` | `authoritative` | `entra-audit` | `entra-audit` |
| App role assignment | `reconstructed` | `authoritative` | `snapshot-diff` | `entra-audit` |
| SP credential — metadata | `authoritative` | `authoritative` | `entra-audit` | `entra-audit` |
| SP credential — secret material | `unavailable` | `unavailable` | — | — |

Two classes use audit-first before-state (Conditional Access, SP
credential metadata). Two classes use snapshot-first (group membership,
app role assignment). Secret material is never in the audit record;
ingestion must never treat its absence as a data defect.

**Rationale (brief):**

- Group membership: WI-05 observed 0/12 events carrying `oldValue`; only
  `newValue` populated (`Group.ObjectID`, `Group.DisplayName`, `SPN`).
- Conditional Access policy: WI-05 observed the full pre-edit policy JSON
  in `oldValue` and the full post-edit policy JSON in `newValue` on a
  single `modifiedProperties` entry. Sufficient to round-trip the policy
  from the audit alone.
- App role assignment: same shape as group membership (rich `newValue`,
  no `oldValue`).
- SP credential metadata: `KeyDescription.oldValue` and `.newValue` both
  present as JSON-array strings of key metadata (`KeyIdentifier`,
  `KeyType`, `KeyUsage`, `DisplayName`). Absence of any `oldValue`
  populated with `"[]"` is the meaningful "empty set" state, not
  "missing data" — see `CONNECTOR_AND_INGESTION_DESIGN.md §23.C`.
- SP credential secret material: `secretText` never appears in the audit
  payload. Phase 1 ingestion maps the field to `unavailable`; recovery
  execution must never attempt to reconstruct a secret.

**Top-level `ConfidenceInfo.level` on `NormalizedChange`:** the per-side
(before / after) asymmetry above does NOT automatically downgrade the
change-level confidence. For a class where before-state is
`reconstructed`, `ConfidenceInfo.level` is still `"high"` when an
authoritative audit event for the change itself exists. The asymmetry is
recorded in `ConfidenceInfo.reasons` and
`ConfidenceInfo.missingFields` (e.g. `"authoritative-before-state"`).

**What this pass does NOT change:** no modification to `StateSnapshot`,
`ConfidenceInfo`, or any other schema type. The enums already enumerate
all four `confidence` values and the `SourceSystem` values we need. This
is a per-class defaults specification, not a schema change.

**Canonical fixture cross-reference:** `platform/fixtures/canonical/`
(generated by `npm run generate-canonical-fixtures` from the WI-05
artifacts) implements this table concretely for the group-membership
class. Any additional canonical fixtures built for the other three
classes must follow the same mapping.
